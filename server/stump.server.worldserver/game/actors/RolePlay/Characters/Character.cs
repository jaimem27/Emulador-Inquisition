    ...
    public sealed class Character : Humanoid, IStatsOwner, IInventoryOwner, ICommandsUser
    {
        ...
        void OnLoggedIn()
        {
            ...
            if (IsFirstConnection)
            {
                ...
                //Item para retomar mazmorra
                var item1 = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(793));
                if (!this.Inventory.HasItem(ItemManager.Instance.TryGetTemplate(793)))
                {
                    item1 = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(793), 1);
                    item1.Effects.Clear();
                    item1.Invalidate();
                    this.Inventory.RefreshItem(item1);
                ...
                //Add Lumino al crear personaje
                //var item = Inventory.TryGetItem(ItemManager.Instance.TryGetTemplate(14961));
                //item = this.Inventory.AddItem(ItemManager.Instance.TryGetTemplate(14961), 1);
                //item.Effects.Add(new EffectInteger(EffectsEnum.Effect_NonExchangeable_981, 1));
                //item.Invalidate();
        ...
        public PartyTeleport TeleportRequest
        {
            get;
            set;
        }
        ...
        private void OnEnergyChanged(short energy, short diff)
        {
            ...
            Record.TreasureSearch = 0;
            Record.TreasureMapCoffre = null;
            Record.TreasureTimeStart = DateTime.Now;
            Record.TreasureIndice = null;
        ...
        public bool IsMountInventoryEmpty()
        {
            ...
            {
                SendServerMessage("Primero debes vaciar el inventario de tu montura, antes de desequiparlo.", Color.OrangeRed);
                return false;
            }
            else
            {
        ...
        public LlaveroCollection LlaveroCollecction { get; private set; }


        public void AddMazmorraTerminada(int idLlave)
        {
            MazmorrasTerminadas = idLlave.ToString();
            MazmorrasTerminadas2.Add(idLlave);
        }

        public List<int> MazmorrasTerminadas2
        {
            get { return m_record.DungeonDone; }
            private set { m_record.DungeonDone = value; }
        }

        public String MazmorrasTerminadas
        {
            get { return m_record.DungeonDoneCSV; }
            private set { m_record.DungeonDoneCSV = value; }
        }
        #endregion


        #region Almanax
        public AlmanaxCollection AlmanaxCollection { get; private set; }

        public void AddAlmanaxRezos(int amount)
        {
            RezosAlmanax += amount;
        }

        public int RezosAlmanax
    {
            get { return m_record.RezosAlmanax; }
            private set { m_record.RezosAlmanax = value; }
        }
        #endregion

        public DopeulCollection DoppleCollection { get; private set; }
        ...
        {
            MuteUntil = null;
            SendServerMessage("Has sido desmuteado.", Color.Red);
        }
        ...
        {
            if (IsPrestigeMax() && PrestigeManager.Instance.PrestigeEnabled)
                return false;
            ...
            OpenPopup($"Acabas de aumentar el artefacto al nivel {PrestigeRank} !\r\n Se ha colocado el Artefacto del Viajero como base. \r\n Recuerda que puedes cambiar de artefacto cuando quieras.");
            RefreshActor();
        ...
        public void SetAgressionPenality(TimeSpan time)
        {
            ...
            SendServerMessage("Has sido baneado por un período de " + ArenaManager.ArenaPenalityTime + " minutos por abandonar un partido .. ", Color.DarkOrange);
        ...
        {
            OpenPopup(message, "Inquisition", 0);
        }
        ...
        public override void OnEnterMap(Map map)
        {
            ...
            if (map.Zaap != null && !KnownZaaps.Contains(map))
                DiscoverZaap(map);
            //Retomar mazmorra actualizar al pasar de mapa
            if (map.IsDungeon())
                CambiarMapaRetomarMazmorra(map.Id);
        ...
        public CompanionActor CreateCompanion(FightTeam team, CharacterFighter characterFighter)
        {
            ...
            var Listcompanion = Singleton<CompanionsManager>.Instance.GetCompanionById(Inventory.GetItems(CharacterInventoryPositionEnum.INVENTORY_POSITION_ENTITY).First().Template.Id);
        ...
        public int RetomarMazmorra
        {
            get { return m_record.RetomarMazmorra; }
            private set { m_record.RetomarMazmorra = value; }
        }

        public void CambiarMapaRetomarMazmorra(int MapaId)
        {
            SetRetomarMazmorra(MapaId);
            RefreshActor();
        }

        public int GetRetomarMazmorra()
        {
            return RetomarMazmorra;
        }

        public void SetRetomarMazmorra(int mapaMazmorra)
        {
            RetomarMazmorra = mapaMazmorra;
        }
        #endregion
            ...
            try
            {
                ...
                lock (SaveSync)
                {
                    using (var transaction = database.GetTransaction())
                    {
                        ...
                        AlmanaxCollection.Save(ServerBase<WorldServer>.Instance.DBAccessor.Database);
                        LlaveroCollecction.Save(ServerBase<WorldServer>.Instance.DBAccessor.Database);
                        SaveMounts();
                        SaveQuests();
                        //Alliance.Save(database);
        ...
        public void LoadRecord()
        {
            ...
            LowerBoundHonor = ExperienceManager.Instance.GetAlignementGradeHonor((byte)AlignmentGrade);
            UpperBoundHonor = ExperienceManager.Instance.GetAlignementNextGradeHonor((byte)AlignmentGrade);
            DoppleCollection = new DopeulCollection();
            DoppleCollection.Load(Id);
            AlmanaxCollection = new AlmanaxCollection();
            AlmanaxCollection.Load(Id);
            Inventory = new Inventory(this);
            Inventory.LoadInventory();
            IdolInventory = new IdolInventory(this);
            LlaveroCollecction = new LlaveroCollection();
            LlaveroCollecction.Load(Id);
            Bank = new Bank(this);
            Bank.LoadRecord();
        ...
        public static int[] SpellsBugs =
      {
        ...
        public CharacterMinimalGuildPublicInformations GetCharacterMinimalGuildPublicInformations () => new CharacterMinimalGuildPublicInformations (
            (ulong) Id,
            Name,
            Level, 0);
        ...
        public DateTime LastTeleportParty
        {
            get;
            set;
        }

        public int TreasureSearch
        {
            get;
            set;
        }

        public int? TreasureMapCoffre
        {
            get;
            set;
        }

        public DateTime TreasureTimeStart
        {
            get;
            set;
        }

        public int? TreasureIndice
        {
            get;
            set;
        }
        ...
        public static implicit operator Character(CharacterRecord v)
        {
            throw new NotImplementedException();
        }
    }
