    ...
    public class LoterieSkill : CustomSkill
    {
        ...
        public override int StartExecute(Character character)
        {
            if (character.WorldAccount.LastLoterieDate != null && character.WorldAccount.LastLoterieDate.Date == DateTime.Now.Date)
            {
                ...
                if (ticket != null)
                {
                    ...
                    character.SendServerMessage("Has utilizado un cupon de loteria ! Shine está rezando para que te toque algo bonito !", Color.YellowGreen);
                    GetPrice(character);
                }
                else
                    character.OpenPopup("La loteria es totalmente gratis, pero necesitas un cupon de loteria.");
            }
            else
            {
                GetPrice(character);
        ...
        private void GetPrice(Character character)
        {
            ...
            if (Sort != null)
            {
                ...
                character.OpenPopup("Has ganado un nuevo objeto, que nervios! ¿Que sera?");
            }
            else
            {
                character.OpenPopup("No has ganado nada, baia, culpa de Shine");
            }
        }
    }
}
