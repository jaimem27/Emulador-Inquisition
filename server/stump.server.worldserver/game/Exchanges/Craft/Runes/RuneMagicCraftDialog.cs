    ...
    public abstract class RuneMagicCraftDialog : BaseCraftDialog
    {
        ...
        private void ApliedEffectsPotion(PlayerTradeItem potion)
        {
            ...
            if (effect != null)
            {
                ...
                var randNumber = (int)(rand.NextDouble() * 50);
        ...
        public void ApplyRune()
        {
            ...
            {
                Crafter.Character.SendServerMessage("No se puede maguear un Objevivo, quitatelo para continuar.");
                return;
            }
            ...
            {
                Crafter.Character.SendServerMessage("No se puede maguear un Mimobionte, quitatelo para continuar.");
                return;
            }
            ...
            {
                Crafter.Character.OpenPopup("El objeto que intentas modificar ya tiene un encantamiento.");
            }
            ...
            if (signature != null)
            {
                ...
                if (signature != null)
                {
                    ...
                    Crafter.Character.SendServerMessage("Has firmado con exito tu objeto :D.");
                }
            }
            ...
            if (orbe != null)
            {
                ...
                {
                    Crafter.Character.OpenPopup("El nivel de este orbe es demasiado bajo para usarlo en este objeto.");
                    OnRuneApplied(CraftResultEnum.CRAFT_SUCCESS, MagicPoolStatus.UNMODIFIED);
                }
                else
                {
            ...
            else if (specialrune != null)
            {
                ...
                {
                    Crafter.Character.OpenPopup("No puedes aplicar esta runa a un objeto con un over o un exo.");
                    return;
                }
            ...
            else if (rune != null)
            {
                ...
                if (ItemToImprove.PlayerItem.Template.IsShield())
                {
                    ...
                    if (RunesShields.Contains(rune.Template.Id))
                    {
                        switch (this.ItemToImprove.Owner.Account.Lang)
                        {
                            case "es":
                                ...
                                break;
                            case "fr":
                                Crafter.Character.OpenPopup("Vous ne pouvez appliquer cet effet qu'à un élément de type bouclier.");
                                break;
                            case "pt":
                                Crafter.Character.OpenPopup("Você só pode aplicar esse efeito a um item de tipo de escudo.");
                                break;
                            default:
                                Crafter.Character.OpenPopup("You can only apply this effect to a shield type item.");
                                break;
                        }
                        OnRuneApplied(CraftResultEnum.CRAFT_SUCCESS, MagicPoolStatus.UNMODIFIED);
                    }
        ...
        private void ApliedEffectsRune(PlayerTradeItem rune)
        {
            foreach (var effect in rune.Effects.OfType<EffectInteger>())
            {
                ...
                var percent = (EffectManager.Instance.GetEffectPower(effect) * ItemToImprove.Template.Level + (Job.Level)) / 100;
                var xp = 10 * EffectManager.Instance.GetEffectPower(effect) - percent;
        ...
        {
            var pwrToLose = (int)Math.Ceiling(EffectManager.Instance.GetEffectPower(runeEffect) * 1.25); //FORJA *2
            short residual = 0;
        ...
        private void GetChances(EffectInteger effectToImprove, EffectInteger runeEffect, out double criticalSuccess, out double neutralSuccess, out double criticalFailure)
        {
            ...
            if (effectToImprove != null && !IsExotic(effectToImprove))
                max = (EffectManager.Instance.GetEffectPower(runeEffect.Template) * GetTemplateEffect(effectToImprove).Max) - (GetTemplateEffect(effectToImprove).Min * 0.35); //FORJA ---
