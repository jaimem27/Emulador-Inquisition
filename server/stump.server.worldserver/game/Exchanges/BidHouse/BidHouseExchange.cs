...
namespace Stump.Server.WorldServer.Game.Exchanges.BidHouse
{
    public class BidHouseExchange : IExchange
    {
        ...
        public SellerBuyerDescriptor GetBuyerDescriptor()
        {
            ...
            return new SellerBuyerDescriptor (BidHouseManager.Quantities.Select(x => (uint)x).ToArray(), Types.Select(x => (uint)x).ToArray(), BidHouseManager.TaxPercent, BidHouseManager.TaxModificationPercent, (byte) MaxItemLevel, Character.Level, -1, (ushort) BidHouseManager.UnsoldDelay);
        }
