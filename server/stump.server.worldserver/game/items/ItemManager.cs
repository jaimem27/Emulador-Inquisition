...
namespace Stump.Server.WorldServer.Game.Items
{
    public class ItemManager : DataManager<ItemManager>
    {
        ...
        {
            DopeulRecord record = new DopeulRecord
            {
        ...
        public void CreateLlavero(Character owner, int llaveId)
        {
            LlaveroRecord record = new LlaveroRecord
            {
                CharacterId = owner.Id,
                LlaveId = llaveId,
                Time = DateTime.Now.Add(new TimeSpan(7, 0, 0, 0)),
                IsNew = true,
            };
            owner.LlaveroCollecction.Llavero.Add(record);
            //Database.Insert(record);
        }

        public void CreateAlmanax(Character owner, int almanaxId)
        {
            AlmanaxRecord record = new AlmanaxRecord
            {
                CharacterId = owner.Id,
                AlmanaxDia = almanaxId,
                Time = DateTime.Now.Add(new TimeSpan(0, 24, 0, 0)),
                IsNew = true,
                Ip = owner.Client.IP
            };
            owner.AlmanaxCollection.Almanax.Add(record);
            //Database.Insert(record);
        }
