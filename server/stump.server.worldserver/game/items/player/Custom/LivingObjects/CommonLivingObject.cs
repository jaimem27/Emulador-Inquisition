...
namespace Stump.Server.WorldServer.Game.Items.Player.Custom.LivingObjects
{
    public abstract class CommonLivingObject : BasePlayerItem
    {
        ...
        protected static short[] LevelsStepsColor = {
            0,
            10,
            21,
            33,
        ...
        public short Level
        {
            ...
            {
                short level = 1;
                if (Template.Id == 19524 || Template.Id == 19525 || Template.Id == 19526)
                {
                    while (LevelsStepsColor.Length > level && LevelsStepsColor[level] <= Experience)
                        level++;
                }
                else
                {
