...
namespace Stump.Server.WorldServer.Game.Items.Player
{
    ...
    public sealed class Inventory : ItemsStorage<BasePlayerItem>, IDisposable
    {
        ...
        public uint WeightTotal => 5000 + (uint)(1 * Owner.Stats.Strength.Total) + (uint)Owner.Stats[PlayerFields.Weight].Total;
        ...
        public void Save(ORM.Database database, bool updateAccount)
        {
            lock (Locker)
            {
                ...
                Owner.Account.Tokens = Tokens == null ? 0 : (int)Tokens.Stack;
                if (updateAccount)
                {
