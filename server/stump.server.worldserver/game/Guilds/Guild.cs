    ...
    public class Guild
    {
        ...
        protected virtual void OnMemberRemoved(GuildMember member)
        {
            ...
            if (Alliance != null)
            {
                ...
                member.Character.SendInformationMessage(TextInformationTypeEnum.TEXT_INFORMATION_MESSAGE, 410,
                    Alliance.Name);
        ...
        public GuildFactsMessage GetGuildFactsMessage()
        {
            if (Alliance != null)
            {
                if (Level > 200)
                    return new GuildInAllianceFactsMessage(
                        new GuildFactSheetInformations((uint)Id, "° " + Name + " °", (byte)(Level - 200),
                            Emblem.GetNetworkGuildEmblem(),
                            (uint)Boss.Id,
                            (ushort)Members.Count),
                        (int)Record.CreationDate.GetUnixTimeStamp(),
                        (ushort)TaxCollectors.Count,
                        (from x in Members
                         select x.IsConnected
                             ? x.Character.GetCharacterMinimalGuildPublicInformations()
                             : new CharacterMinimalGuildPublicInformations((uint)x.Id, x.Name,
                                 (ushort)Singleton<ExperienceManager>.Instance.GetCharacterLevel(x.Experience), 0)).ToList(),
                        Alliance.GetBasicNamedAllianceInformations());
                else
                    return new GuildInAllianceFactsMessage(
                        new GuildFactSheetInformations((uint)Id, Name, (byte)Level, Emblem.GetNetworkGuildEmblem(),
                            (uint)Boss.Id,
                            (ushort)Members.Count),
                        (int)Record.CreationDate.GetUnixTimeStamp(),
                        (ushort)TaxCollectors.Count,
            ...
            if (Level > 200)
                return new GuildFactsMessage(
                    new GuildFactSheetInformations((uint)Id, "° " + Name + " °", (byte)(Level - 200),
                        Emblem.GetNetworkGuildEmblem(), (uint)Boss.Id,
                        (ushort)Members.Count),
                    (int)Record.CreationDate.GetUnixTimeStamp(),
                    (ushort)TaxCollectors.Count,
                    (from x in Members
