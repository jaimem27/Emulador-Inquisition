...
namespace Stump.Server.WorldServer.Game.Fights
{
    ...
    {
        [Variable]
        public static int PlacementPhaseTime = 300000;
        ...
        where TBlueTeam : FightTeam
    {
        ...
        public static List<int> AscensionMaps = new List<int>()
        {
          196215814,
          77332993,
          77332995,
        ...
        {
            leaver.Character.Energy -= 1000;
            #region RetomarMazmorra
            if (Map.IsDungeon())
            {
                leaver.Character.CambiarMapaRetomarMazmorra(Map.Id);
                leaver.Character.SaveLater();
            }
            else
            {
                leaver.Character.CambiarMapaRetomarMazmorra(0);
                leaver.Character.SaveLater();
            }
            leaver.Character.SendServerMessage("Has abandonado");
            #endregion
        ...
        {
            int coe = (int)Math.Round(ActiveIdols.Sum(x => x.DropBonus * x.GetSynergy(ActiveIdols)));
        ...
        public FightCommonInformations GetFightCommonInformations()
        {
            return new FightCommonInformations((ushort)Id,
                ...
                m_teams.Select(entry => (ushort)entry.BladePosition.Cell.Id),
        ...
        public FightExternalInformations GetFightExternalInformations(Character character)
        {
            return new FightExternalInformations(
                (ushort)Id,
                (sbyte)FightType,
                (int)(!IsStarted ? 0 : StartTime.GetUnixTimeStamp()),
                SpectatorClosed,
                m_teams.Select(entry => entry.GetFightTeamLightInformations(character)).ToArray(),
