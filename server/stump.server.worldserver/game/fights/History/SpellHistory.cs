...
namespace Stump.Server.WorldServer.Game.Fights.History
{
    public class SpellHistory
    {
        ...
        public void ReducCooldown(int spell, int nbrTurn)
        {
            SpellHistoryEntry spellHistoryEntry = this.m_underlyingStack.LastOrDefault((SpellHistoryEntry entry) => entry.Spell.SpellId == spell);
            if (spellHistoryEntry != null)
            {
                if (!spellHistoryEntry.IsGlobalCooldownActive(this.CurrentRound))
                {
                    spellHistoryEntry.CastRound = CurrentRound - nbrTurn;
                }
            }
        }
    }
