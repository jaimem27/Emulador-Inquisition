    ...
    public class FightAgression : Fight<FightPlayerTeam, FightPlayerTeam>
    {
        ...
        private void CalculateEarnedRank(FightPlayerResult result)
        {
...

            {
                ...
                {
                    winPoints = 5;
                    result.Character.SendServerMessage("Tu oponente se rindió demasiado rápido. Por lo tanto, lamentablemente solo ganarás <b> " + winPoints + " </b> CP durante esta pelea", Color.Chartreuse);
                }
                else
                {
                    {
                    ...
                    result.Character.SendServerMessage("Has ganado <b>" + winPoints + "</b> CP durante este combate", Color.Chartreuse);
                }
            ...
            else
            {
                ...
                if (winner != null)
                {
                    ...
                    result.Character.SendServerMessage("Has perdido <b>" + lostPoint + "</b> CP durante el combate.", Color.Chartreuse);
        ...
        private void CalculateEarnedKolizetons(FightPlayerResult result)
        {
            ...
            if (target != null)
            {
                ...
                if (Winners == result.Fighter.Team)
                {
                    ...
                    result.Character.SendServerMessage("Has ganado el combate, ¡Felicidades!");
        ...
        private void SendFightInformationsOnFinish(bool real)
        {
            ...
            if (!real)
            {
                foreach (var fighter in this.GetAllCharacters())
                    fighter.SendServerMessage("Debido a la duración demasiado corta de la pelea, algunas estadísticas han cambiado.", System.Drawing.Color.DarkOrange);
            }
            ...
            if (m_winnerscount == 1 && m_loserscount == 1)
            {
                if (battleField)
                {
                    World.Instance.SendAnnounce($"El jugador <b>{(Winners.Fighters[0] as CharacterFighter).Character.Name}</b> ha ganado a <b>{(Losers.Fighters[0] as CharacterFighter).Character.Name}</b> en DeathMatch." +
                        $" La batalla ha durado <b>{this.GetFightDuration().Minutes}</b> minutos y <b>{this.GetFightDuration().Seconds}</b> segundos.", Color.YellowGreen);
                }
                else
                {
                    World.Instance.SendAnnounce($"El jugador <b>{(Winners.Fighters[0] as CharacterFighter).Character.Name}</b> ha ganado a <b>{(Losers.Fighters[0] as CharacterFighter).Character.Name}</b> en combate PvP." +
                        $" La batalla ha durado <b>{this.GetFightDuration().Minutes}</b> minutos y <b>{this.GetFightDuration().Seconds}</b> segundos.", Color.YellowGreen);
                }
            }
                ...
                if (pvpSeek.Effects.FirstOrDefault(x => x.EffectId == EffectsEnum.Effect_Seek) is EffectString seekEffect)
                {
                    ...
                    if (target != null)
                    {
                        ...
                        else
                        {
                            ...
                            target.Character.SendServerMessage("Has ganado el combate, ¡Felicidades!");
                        }
