...
namespace Stump.Server.WorldServer.Game
{
    public class World : DataManager<World>
    {
        ...
        {
            var mapa = World.Instance.GetMap(currentMap.Id);
            var actualPoint = new MapPoint(mapa.Position);
        ...
        public void Save()
        {
            lock (SaveLock)
            {
                ...
                if (WorldServer.SaveMessage)
                    SendAnnounce("Guardando el servidor.", Color.OrangeRed);
                ...
                if (WorldServer.SaveMessage)
                    SendAnnounce("Se ha guardado.",Color.OrangeRed);
