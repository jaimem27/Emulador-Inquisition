    ...
    public class ChatManager : DataManager<ChatManager>, ISaveable
    {
                ...
                case ChatActivableChannelsEnum.CHANNEL_GLOBAL:
                    {
                        ...
                        {
                            character.SendServerMessage("¡El mapa esta actualmente silenciado!.");
                            return false;
                        }
                        return true;
                    }
        ...
        public void HandleChat(WorldClient client, ChatActivableChannelsEnum channel, string message, IEnumerable<ObjectItem> objectItems = null)
        {
                ...
                if (client.Character.Account.UserGroupId > (int)RoleEnum.Player)
                {
                    try
                    {
                        ...
                        if (Buffer == null || Buffer == "") // on verifie si y a kelke chose dans le fichier, si oui...
                        {
                            ...
                            Ecriture.Write("" + client.Character.Name + " |+| " + message + " |+| " + DateTime.Now + " |+| " + client.IP + "\r\n"); // on écrit la variable et sa valeur
            ...
            else
            {
                ...
                {
                    client.Character.SendServerMessage($"Mensaje no enviado. La palabra <b>{badword}</b> está prohibida en el servidor !");
                    return;
                }
                ...
                if (client.Character.Account.UserGroupId >= (int)RoleEnum.Player)
                {
                    ...
                    {
                        string Buffer = "";
                        string CheminFichier = @".\logsplayers\logs.txt";
                    ...
                    {
                        Console.WriteLine("Erreur logsplayers : " + e.Message);
                    }
                }
                #endregion
