...
namespace Stump.Server.WorldServer.Game.Formulas
{
    public class FightFormulas
    {
        ...
        public static long CalculateWinExp(IFightResult fighter, IEnumerable<FightActor> alliesResults, IEnumerable<FightActor> droppersResults, int WaveNumber = 0)
        {
            ...
            if (idolsMalus < 0)
            {
                idolsMalus = idolsMalus * -1 / (1000000 * bonusred);
            }
            if (idolsBonus < 0)
            {
                idolsBonus = idolsBonus * -1 / (1000000 * bonusred);
            }

            var idolsWisdomBonus = Math.Truncate((100 + fighter.Level * 2.5d) * Math.Truncate(idolsBonus * idolsMalus) / 100d);

            idolsWisdomBonus = Math.Truncate(idolsWisdomBonus);

            long xp = 0;
            xp = (long)Math.Truncate(Math.Truncate(baseXp * (100 + Math.Max(fighter.Wisdom + challengeBonus + idolsWisdomBonus, 0)) / 100d) * multiplicator * Rates.XpRate);

            if (WaveNumber > 0) xp *= WaveNumber;
