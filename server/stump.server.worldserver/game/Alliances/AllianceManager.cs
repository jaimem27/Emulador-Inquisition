...
namespace Stump.Server.WorldServer.Game.Alliances {
    public class AllianceManager : DataManager<AllianceManager>, ISaveable {
        ...
        public void Save () {
            lock (m_lock) {
                ...
                using (var enumerator = (
                    from alliance in m_alliances.Values where alliance.IsDirty select alliance).GetEnumerator ()) {
        ...
        public override void Initialize () {
            m_emblems = Database.Query<Database.Guilds.EmblemRecord> (Server.WorldServer.Database.Guilds.EmblemRelator.FetchQuery).ToDictionary (x => x.Id);
            ...
            if (!m_alliances.Any ()) {
                ...
                m_idProvider = new UniqueIdProvider ((
                    from temp in m_alliances select temp.Value.Id).Max ());
            }
                        ...
                        if (basePlayerItem == null) {
                            ...
                            if (alliance == null) {
                                ...
                                if (!alliance.TryAddGuild (character.Guild)) {
                                    ...
                                    var document = new BsonDocument { { "characterId", character.Id }, { "characterName", character.Name }, { "allianceName", allianceName }, { "GuildID", character.Guild.Id }, { "emblembackgroundColor", emblem.BackgroundColor }, { "emblembackgroundShape", emblem.BackgroundShape }, { "emblemsymbolColor", emblem.SymbolColor }, { "emblemsymbolShape", emblem.SymbolShape }, { "Date", DateTime.Now.ToString (CultureInfo.InvariantCulture) }
                                    };
