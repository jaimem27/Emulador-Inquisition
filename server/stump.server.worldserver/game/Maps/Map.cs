    ...
    public class Map : WorldObjectsContext, ICharacterContainer
    {
        ...
        public void Enter(RolePlayActor actor)
        {
            ...
            if (actor is Character)
            {
                ...
                if (IsInstantiated)
                    (actor as Character).SendServerMessage("Cette carte est soumise � une instance temporelle, X�lor vous restreind le fait de pouvoir int�ragir avec d'autres joueurs !");
            }
            
#pragma warning disable CS0184 // La expresi�n dada nunca es del tipo proporcionado ('Prism')
            OnActorEnter(actor);
            if (actor is Prism) {
#pragma warning restore CS0184 // La expresi�n dada nunca es del tipo proporcionado ('Prism')
                if (!Clients.Contains ((actor as Character).Client)) {
                    PrismHandler.SendPrismsListMessage ((actor as Character).Client);
                }
            }
            Refresh(actor);
        }
