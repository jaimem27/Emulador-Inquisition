        ...
        protected override bool InternalApply()
        {
            ...
            foreach (var target in GetAffectedActors())
            {
                ...
                else if (Spell.Id == 9956)
                {
                    ...
                    var CaC = casterPoint.ManhattanDistanceTo(TargetedCell);

                    // fix bug sacrogrito
                    if (CaC > 1)
                    {
                        if (caster.Direction == DirectionsEnum.DIRECTION_SOUTH_EAST && caster.Direction == pushDirection)
                        {
                            caster.Direction = pushDirection;
                            var cellsacro = new MapPoint((targetPoint.X - 1), (targetPoint.Y));
                            var destsacro = Map.GetCell(cellsacro.CellId);

                            caster.Position.Cell = destsacro;
                            Fight.ForEach(entry => ActionsHandler.SendGameActionFightTeleportOnSameMapMessage(entry.Client, caster, caster, destsacro), true);
                        }
                        else if (caster.Direction == DirectionsEnum.DIRECTION_SOUTH_WEST && caster.Direction == pushDirection)
                        {
                            caster.Direction = pushDirection;
                            var cellsacro = new MapPoint((targetPoint.X), (targetPoint.Y + 1));
                            var destsacro = Map.GetCell(cellsacro.CellId);

                            caster.Position.Cell = destsacro;
                            Fight.ForEach(entry => ActionsHandler.SendGameActionFightTeleportOnSameMapMessage(entry.Client, caster, caster, destsacro), true);
                        }
                        else if (caster.Direction == DirectionsEnum.DIRECTION_NORTH_EAST && caster.Direction == pushDirection)
                        {
                            caster.Direction = pushDirection;
                            var cellsacro = new MapPoint((targetPoint.X), (targetPoint.Y - 1));
                            var destsacro = Map.GetCell(cellsacro.CellId);
                        ...
                        else if (caster.Direction == DirectionsEnum.DIRECTION_NORTH_WEST && caster.Direction == pushDirection)
                        {
                            caster.Direction = pushDirection;
                            var cellsacro = new MapPoint((targetPoint.X + 1), (targetPoint.Y));
                            var destsacro = Map.GetCell(cellsacro.CellId);
