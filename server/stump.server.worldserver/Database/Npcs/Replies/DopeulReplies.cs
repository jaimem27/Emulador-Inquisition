            ...
            if (compareTime <= DateTime.Now)
            {
                ...
                if (!(compareTime <= DateTime.Now))
                {
                    switch (character.Account.Lang)
                    {
                        ...
                        $"Espera que no puedas hacer esto, ya peleaste con un dopeul en alguna de tus cuentas!, Tienes que esperar <b>{compareTime.Subtract(DateTime.Now).Hours} horas, {compareTime.Subtract(DateTime.Now).Minutes} minutos</b>",
                        Color.Red);
                            break;
                        case "fr":
                            character.SendServerMessage(
                ...
                if (EditDopple != null)
                {
                    ...
                    EditDopple.Time = DateTime.Now.AddHours(24);
                }
                else
                {
