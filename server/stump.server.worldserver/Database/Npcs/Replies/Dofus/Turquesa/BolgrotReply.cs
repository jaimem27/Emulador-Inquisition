...
{
    [Discriminator("BolgrotReply", typeof(NpcReply), new System.Type[]
 {
  typeof(NpcReplyRecord)
 })]
    class BolgrotReply : NpcReply
    {
        public BolgrotReply(NpcReplyRecord record)
            : base(record)
        {
        }
        ...
        public override bool Execute(Npc npc, Character character)
        {
            ...
            var grade = Singleton<MonsterManager>.Instance.GetMonsterGrade(4708, 5);
            ...
            var grade1 = Singleton<MonsterManager>.Instance.GetMonsterGrade(5019, 5);
            var monster1 = new Monster(grade1, new MonsterGroup(0, position));

            var grade2 = Singleton<MonsterManager>.Instance.GetMonsterGrade(4708, 2);
            var monster2 = new Monster(grade2, new MonsterGroup(0, position));

            var grade3 = Singleton<MonsterManager>.Instance.GetMonsterGrade(5208, 2);
            var monster3 = new Monster(grade3, new MonsterGroup(0, position));

            var grade4 = Singleton<MonsterManager>.Instance.GetMonsterGrade(5401, 2);
