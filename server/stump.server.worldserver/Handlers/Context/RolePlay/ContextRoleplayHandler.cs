...
namespace Stump.Server.WorldServer.Handlers.Context.RolePlay
{
    public partial class ContextRoleplayHandler : WorldHandlerContainer
    {
        ...
        {
            if (client.Character.Map.Id == ArenaManager.KolizeumMapId)
                SendMapRunningFightListMessage(client, ArenaManager.Instance.Arenas_1vs1.SelectMany(x => x.Value.Map.Fights).Concat(ArenaManager.Instance.Arenas_3vs3.SelectMany(x => x.Value.Map.Fights)),
                    client.Character);
            else
        ...
        {
            client.Send(
                new MapRunningFightListMessage(fights.Select(entry => entry.GetFightExternalInformations(character))));
        }
