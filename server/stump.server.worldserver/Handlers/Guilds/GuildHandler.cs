...
namespace Stump.Server.WorldServer.Handlers.Guilds
{
    public class GuildHandler : WorldHandlerContainer
    {
        ...
        public static void HandleGuildFactsRequestMessage(WorldClient client, GuildFactsRequestMessage message)
        {
            ...
            {
                client.Send(new GuildInAllianceFactsMessage(guild.GetGuildFactSheetInformations(),
                    (int)guild.CreationDate.GetUnixTimeStamp(), (ushort)guild.TaxCollectors.Count,
                    guild.Members.Select(x => x.GetCharacterMinimalGuildPublicInformations()),
                    guild.Alliance.GetBasicNamedAllianceInformations()));
            }
            else
