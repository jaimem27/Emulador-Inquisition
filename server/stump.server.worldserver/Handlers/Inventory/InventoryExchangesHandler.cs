...
namespace Stump.Server.WorldServer.Handlers.Inventory
{
    public partial class InventoryHandler : WorldHandlerContainer
    {
        ...
        public static void HandleExchangeBidHouseTypeMessage(WorldClient client, ExchangeBidHouseTypeMessage message)
        {
            ...
            if (message.Type == 0)
            {
                foreach (var type in exchange.Types)
                {
                    items.AddRange(BidHouseManager.Instance.GetBidHouseItems((ItemTypeEnum)type, exchange.MaxItemLevel).ToArray());
                }
            }
            else
            {
                items = BidHouseManager.Instance.GetBidHouseItems((ItemTypeEnum) message.Type, exchange.MaxItemLevel).ToList();
            }
        ...
        public static void HandleExchangeBidHouseBuyMessage(WorldClient client, ExchangeBidHouseBuyMessage message)
        {
            ...
            {
                //Action annul�e pour cause de surcharge...
        ...
        {
            client.Send(new ExchangeStartedTaxCollectorShopMessage(taxCollector.Bag.Select(x => x.GetObjectItem()).ToList(), (ulong)taxCollector.GatheredKamas));
        }
